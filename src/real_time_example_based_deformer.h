...
public:
    ...
    void setReconstructEigenfunctionNum(int num) {reconstruct_eigenfunction_num_=num;}
...
private:
    ...
    void reconstructFromEigenCoefs(Vec3d *target_eigencoefs,int eigenfunction_num,double *vert_pos);//full space
    void projectOnEigenFunctionsInLocalFrame(double *displacement, double *vertex_volume,double **eigenfunctions,
                                            double *eigenvalues, unsigned int eigenfunction_num,Vec3d *eigencoefs);
    void projectOnObjectEigenFunctionsInLocalFrame(double *displacement,unsigned int eigenfunction_num,Vec3d *eigencoefs);
    void projectOnExampleEigenFunctionsInLocalFrame(double *init_pos, int vert_num,
    												double *vertex_volume,double **eigenfunctions, double *eigenvalues,
    												unsigned int eigenfunction_num,Vec3d *eigencoefs);
    void reconstructFromEigenCoefsInLocalFrame(Vec3d *target_eigencoefs,int eigenfunction_num,double *vert_pos);
    void reconstructByLocalFrameBuildOnVert(Vec3d *target_eigencoefs,int flag=0);//reduced_space
    void saveReconstructMesh(double *vert_pos,std::string file_name);
    void saveReconstructMesh1(double *vert_pos,const std::string &file_name);//for test,nothing important
    ...
    void reconstructFromReducedBasis(Vec3d *target_eigencoefs,double *vert_pos);
...
private:
    ...
    double new_R_[9];
    double t_[3];
    double **example_local_reference_=NULL;
