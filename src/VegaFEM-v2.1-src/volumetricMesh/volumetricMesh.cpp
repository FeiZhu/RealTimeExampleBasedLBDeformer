...
void VolumetricMesh::computeGravity(double * gravityForce, double g, bool addForce) const
{
  ...
  for(int el=0; el < numElements; el++)
  {
    ...
    {
        // gravityForce[3 * getVertexIndex(el,j) + 0] -= invNumElementVertices * mass * g; // gravity assumed to act in negative y-direction
        gravityForce[3 * getVertexIndex(el,j) + 1] -= invNumElementVertices * mass * g; // gravity assumed to act in negative y-direction
        // gravityForce[3 * getVertexIndex(el,j) + 2] -= invNumElementVertices * mass * g; // gravity assumed to act in negative y-direction
    }
    //   gravityForce[3 * getVertexIndex(el,j) + 1] -= invNumElementVertices * mass * g; // gravity assumed to act in negative y-direction
  }
}
